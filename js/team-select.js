...
const TeamSelect = (() => {
  ...
  I18n.addDict({
    pt: {
      'ts.title': 'ESCOLHA SEU TIME',
      'ts.searchPlaceholder': 'Buscar nome ou texto de habilidade...',
      'ts.prevChar': 'Personagem anterior',
      'ts.nextChar': 'Próximo personagem',
      'ts.close': 'Fechar',
      'ts.noMatch': 'Nenhum personagem corresponde à sua busca.',
      'ts.addToDeck': 'Adicionar ao Time',
      'ts.yourDeck': 'Seu Time',
      'ts.startBattle': 'Iniciar Batalha',
      'ts.empty': 'Vazio',
      'ts.spd': 'VEL',
      'ts.spdTitle': 'Velocidade de turno desta habilidade',
      'ts.passive': 'Passiva',
      'ts.passiveTitle': 'Passiva \u2014 sempre ativa',
      'ts.passiveNoSpeedTitle': 'Habilidades passivas não têm velocidade e não podem ser selecionadas',
      'ts.tapToggleForm': 'Toque para ver sua outra forma',
      'ts.tapPreviewAbility': 'Toque para ver a animação desta habilidade',
      'ts.inDeck': 'No Time \u2713 (toque para remover)',
      'ts.roleFull': 'Papel Completo ({cap}/{cap})',
      'ts.summonBadge': '\u2726 Invoca um token'
    },
    en: {
      'ts.title': 'CHOOSE YOUR TEAM',
      'ts.searchPlaceholder': 'Search name or ability text...',
      'ts.prevChar': 'Previous character',
      'ts.nextChar': 'Next character',
      'ts.close': 'Close',
      'ts.noMatch': 'No characters match your search.',
      'ts.addToDeck': 'Add to Deck',
      'ts.yourDeck': 'Your Deck',
      'ts.startBattle': 'Start Battle',
      'ts.empty': 'Empty',
      'ts.spd': 'SPD',
      'ts.spdTitle': 'Turn-order speed for this ability',
      'ts.passive': 'Passive',
      'ts.passiveTitle': 'Passive \u2014 always active',
      'ts.passiveNoSpeedTitle': "Passive abilities have no speed and can't be selected",
      'ts.tapToggleForm': 'Tap to preview his other form',
      'ts.tapPreviewAbility': "Tap to preview this ability\u2019s animation",
      'ts.inDeck': 'In Deck \u2713 (tap to remove)',
      'ts.roleFull': 'Role Full ({cap}/{cap})',
      'ts.summonBadge': '\u2726 Summons a token'
    }
  });
  ...
  function openTokenPopup(defId) {
    ...
    document.getElementById('ts-token-popup-name').textContent = UnitText.displayName(sDef);
    ...
    (sDef.abilities || []).forEach(a => {
      ...
      row.innerHTML = `
        <div class="ts-ability-name-row">
          <div class="ts-ability-name">${escapeHtml(UnitText.abilityName(sDef, a))}</div>
          <div class="ts-ability-speed" title="${escapeHtml(I18n.t('ts.spdTitle'))}">${escapeHtml(I18n.t('ts.spd'))} ${escapeHtml(String(a.speed))}</div>
        </div>
        <div class="ts-ability-desc">${escapeHtml(UnitText.abilityDesc(sDef, a) || '')}</div>
      `;
  ...
  function ensureMounted() {
    ...
    mount.innerHTML = `
      <div id="team-select" class="ts-root">
        <div class="ts-panel">
        <div class="ts-header">
          <h1>${I18n.t('ts.title')}</h1>
          <div class="ts-tabs" id="ts-tabs"></div>
          <div class="ts-search-wrap">
            <input id="ts-search" class="ts-search" type="text" placeholder="${escapeHtml(I18n.t('ts.searchPlaceholder'))}" autocomplete="off" />
          </div>
        </div>

        <div class="ts-body">
          <div class="ts-stage">
            <button class="ts-arrow ts-arrow-left" id="ts-arrow-left" aria-label="${escapeHtml(I18n.t('ts.prevChar'))}">&#10094;</button>
            ...
            <div class="ts-card" id="ts-card">
              ...
              <div class="ts-ability-panel" id="ts-ability-panel">
                ...
                <button id="ts-add-btn" class="ts-add-btn">${I18n.t('ts.addToDeck')}</button>
              </div>
            </div>

            <button class="ts-arrow ts-arrow-right" id="ts-arrow-right" aria-label="${escapeHtml(I18n.t('ts.nextChar'))}">&#10095;</button>
            <div class="ts-empty" id="ts-empty">${I18n.t('ts.noMatch')}</div>
            <div class="ts-position" id="ts-position"></div>
          </div>

          <div class="ts-deck" id="ts-deck">
            <div class="ts-deck-title">${I18n.t('ts.yourDeck')}</div>
            <div class="ts-deck-slots" id="ts-deck-slots"></div>
            <button id="team-select-confirm" class="ts-confirm">${I18n.t('ts.startBattle')}</button>
          </div>
        </div>
        </div>
      </div>
      ...
      <div class="ts-token-popup" id="ts-token-popup">
        <div class="ts-token-popup-card">
          <button class="ts-token-popup-close" id="ts-token-popup-close" aria-label="${escapeHtml(I18n.t('ts.close'))}">&#10005;</button>
  ...
  function renderDeck() {
    ...
    state.roleOrder.forEach(role => {
      ...
      for (let i = 0; i < 2; i++) {
        ...
        // Always render the HP line, even empty, so a slot's height/row
        // layout doesn't change shape the moment it goes from empty to
        // filled — an empty slot used to omit this line entirely, which
        // made the whole ts-deck-pair row (and anything below it) jump
        // and reflow on mobile's row-stacked layout as picks were added.
        slot.innerHTML = `
          <div class="ts-deck-name">${def ? escapeHtml(UnitText.displayName(def)) : I18n.t('ts.empty')}</div>
          <div class="ts-deck-hp">${def ? `HP ${def.stats.maxHP}` : '\u00A0'}</div>
        `;
        if (def) {
  ...
  function renderCard() {
    ...
    document.getElementById('ts-card-name').textContent = UnitText.displayName(def);
    ...
    (def.abilities || []).forEach(a => {
      ...
      row.title = isPassive
        ? (def.id === 'dario' ? I18n.t('ts.tapToggleForm') : I18n.t('ts.passiveTitle'))
        : I18n.t('ts.tapPreviewAbility');
      const speedBadge = isPassive
        ? `<div class="ts-ability-speed" title="${escapeHtml(I18n.t('ts.passiveNoSpeedTitle'))}">${escapeHtml(I18n.t('ts.passive'))}</div>`
        : `<div class="ts-ability-speed" title="${escapeHtml(I18n.t('ts.spdTitle'))}">${escapeHtml(I18n.t('ts.spd'))} ${escapeHtml(String(a.speed))}</div>`;
      row.innerHTML = `
        <div class="ts-ability-name-row">
          <div class="ts-ability-name">${escapeHtml(UnitText.abilityName(def, a))}</div>
          ${speedBadge}
        </div>
        <div class="ts-ability-desc">${escapeHtml(UnitText.abilityDesc(def, a) || '')}</div>
      `;
    ...
    if (summonDefId) {
      ...
      summonNoteEl.innerHTML = `
        <div class="ts-summon-badge">${escapeHtml(I18n.t('ts.summonBadge'))}</div>
        <div class="ts-summon-name">${escapeHtml(UnitText.displayName(sDef))} \u00b7 HP ${sDef.stats.maxHP}</div>
      `;
    } else {
    ...
    addBtn.textContent = isCurrentPick ? I18n.t('ts.inDeck') : (roleFull ? I18n.t('ts.roleFull', { cap }) : I18n.t('ts.addToDeck'));
    addBtn.classList.toggle('active-pick', isCurrentPick);
    addBtn.classList.toggle('disabled-pick', !isCurrentPick && roleFull);
  }

  I18n.onChange(() => {
    // Re-render whichever team-select surface is currently open, in the
    // now-current language. mounted stays true across a language switch
    // (the DOM shell is torn down and rebuilt, not the module state).
    const root = document.getElementById('team-select');
    const wasOpen = root && root.classList.contains('show');
    const openTokenId = state && state._summonDefId
      && document.getElementById('ts-token-popup')
      && document.getElementById('ts-token-popup').classList.contains('show')
      ? state._summonDefId : null;
    if (!wasOpen) return;
    const savedState = state;
    if (savedState.preview) savedState.preview.destroy();
    mounted = false;
    ensureMounted();
    state = savedState;
    // ensureMounted() rebuilds ts-card-render fresh, so the CardPreview
    // instance from before this rebuild is now pointing at a detached
    // canvas host — build a new one against the new DOM the same way
    // open() does on first use, then re-render everything into it.
    state.preview = new CardPreview(document.getElementById('ts-card-render'));
    renderTabs();
    renderDeck();
    recomputeFiltered();
    ...
    requestAnimationFrame(() => state.preview.resize());
    if (openTokenId) openTokenPopup(openTokenId);
  });
