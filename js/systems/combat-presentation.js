...
(() => {
  ...
  function singleStrike(casterUid,targetUid){
    ...
    ],{duration:1800,easing:'cubic-bezier(.2,.76,.16,1)',fill:'forwards'}).finished.finally(()=>{
      source.setAttribute('style',previousStyle);
      if(!previousStyle)source.removeAttribute('style');
      else source.style.zIndex=previousZ;
    });
    const sword=glyph(s.x,s.y,'damage','hv-sword-projectile',1700);
    sword.animate([
      {transform:'translate(-50%,-50%) scale(.2) rotate(-35deg)',opacity:0},
      {transform:`translate(${(t.x-s.x)*.5}px,${(t.y-s.y)*.5}px) scale(1) rotate(5deg)`,opacity:1,offset:.45},
      {transform:`translate(${t.x-s.x}px,${t.y-s.y}px) scale(1.25) rotate(35deg)`,opacity:1,offset:.78},
      {transform:`translate(${t.x-s.x}px,${t.y-s.y}px) scale(.55) rotate(60deg)`,opacity:0}
    ],{duration:1350,easing:'cubic-bezier(.18,.8,.15,1)',fill:'forwards'});
  ...
  function playCombatSequence(diff){if(!diff)return;const caster=getUnit(diff.casterUid),ability=CARD_DB[caster?.cardId]?.abilities?.[diff.abilityIdx];if(!caster||!ability)return;const first=kindOf(ability.effects?.[0]);actionCue(caster.uid,first);let delay=ACTION_CUE_MS+PRE_ACTION_MS;for(const effect of ability.effects||[]){animateEffect(effect,diff,delay);delay+=STEP_MS}}
