...
(() => {
  ...
  if (typeof window.initGame === 'function' && !window.__hvInitGameWrapped) {
    const nativeInitGame = window.initGame;
    window.__hvInitGameWrapped = true;
  ...
  if (typeof window.executeAbility === 'function' && !window.__hvExecuteWrapped) {
    const nativeExecute = window.executeAbility;
    window.__hvExecuteWrapped = true;
  ...
  if (typeof window.autoResolveStep === 'function' && !window.__hvAutoResolveWrapped) {
    const nativeAutoResolveStep = window.autoResolveStep;
    window.__hvAutoResolveWrapped = true;
    window.autoResolveStep = function() {
      ensureTurnState();
      const item = state?.resolutionQueue?.[state.resolutionIdx];
      if (item && state.hvSkipResolution?.has(item.uid)) {
        state.hvSkipResolution.delete(item.uid);
        state.resolutionIdx++;
        render();
        setTimeout(window.autoResolveStep, 300);
        return;
      }
      if (state?.hvAnimationLock) return;
      return nativeAutoResolveStep();
    };
  }

  if (typeof window.playCombatSequence === 'function' && !window.__hvPresentationWrapped) {
    const nativePlayCombatSequence = window.playCombatSequence;
    window.__hvPresentationWrapped = true;
    window.playCombatSequence = function(diff) {
      ensureTurnState();
      state.hvAnimationLock = true;
      nativePlayCombatSequence(diff);
      const caster = getUnit(diff?.casterUid);
      const ability = CARD_DB[caster?.cardId]?.abilities?.[diff?.abilityIdx];
      const count = Math.max(1, ability?.effects?.length || 1);
      const duration = 2000 + ((count - 1) * 3200) + 2400 + 250;
      setTimeout(() => {
        state.hvAnimationLock = false;
        ...
        render();
        setTimeout(window.autoResolveStep, 120);
      }, duration);
    };
  }
  ...
  if (typeof window.finishResolutionPhase === 'function' && !window.__hvFinishWrapped) {
    window.__hvFinishWrapped = true;
