...
function renderFloatingNumbers(result) {
  ...
  for (const a of (result.applied || [])) {
    ...
    if (a.verb === 'damage' && a.actualDamage > 0) {
      ...
      spawnFloatText(screenPos, "Purificado", "heal");
...
function logResult(result) {
  ...
  for (const eff of (result.applied || [])) {
    ...
    if (eff.verb === 'damage' && eff.actualDamage > 0) {
      ...
      addLogLine(eff.hadStatuses || eff.hadCounters ? `  → ${t} is purified (all gained effects removed)` : `  → ${t} had nothing to purify`, 'info');
    }
  }
}
