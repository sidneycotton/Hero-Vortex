...
function renderOrderStrip(allActions, activeIndex) {
  ...
  allActions.forEach((a, i) => {
    ...
    chip.textContent = `⚡${a.ability.speed} ${UnitText.displayName(a.actor)}`;
...
async function beginResolutionPhaseV2Inner() {
  ...
  addLogLine(I18n.t('log.roundBegins', { count: allActions.length }), 'info');
  ...
  while (i < allActions.length) {
    ...
    if (!target.alive || hasStatus(target, 'untargetable')) {
      ...
      if (!target) {
        addLogLine(I18n.t('log.abilityFizzled', { unit: UnitText.displayName(action.actor), ability: UnitText.abilityName(action.actor, action.ability) }), 'info');
    ...
    if (bonusFires > 0) {
      addLogLine(I18n.t('log.rewriteBonusFires', { unit: UnitText.displayName(action.actor), ability: UnitText.abilityName(action.actor, action.ability), count: bonusFires }), 'info');
      refreshAllUnitUI();
    }

    updateStatus(I18n.t('log.actorUsesAbility', { speed: action.ability.speed, unit: UnitText.displayName(action.actor), ability: UnitText.abilityName(action.actor, action.ability) }));
  ...
  for (const u of allLiving) {
    ...
    for (const er of tickResults) {
      ...
      if (applied.actualDamage > 0) {
        const label = er.source === 'punishIfIgnored' ? I18n.t('log.followupLabel', { unit: UnitText.displayName(u) }) : I18n.t('log.bleedLabel');
        addLogLine(I18n.t('log.bleedTick', { target: UnitText.displayName(er.target), amount: applied.actualDamage, label }), 'hit');
  ...
  await resolveForcedReplacements(playerUnits, playerHand, "player", 'pv.side.your');
  await resolveForcedReplacements(enemyUnits, enemyHand, "enemy", 'pv.side.enemy');
