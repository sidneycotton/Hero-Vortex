...
class Unit {
  ...
  constructor(def, slotIndex, team) {
    this.defId = def.id;
...
async function resolveForcedReplacements(units, hand, team, sideLabel) {
  for (const role of TeamSelect.ROLE_ORDER) {
    ...
    refreshAllUnitUI();
    renderPartyRow();
    await showReinforcementBanner(`${sideLabel} ${roleLabelForBanner(role)} fell — ${unit.displayName} takes the field!`);
  }
}

// Small local label helper for the banner text (StarterSelect/TeamSelect
// each have their own private copy of this same one-liner — kept here
// too rather than reaching into either module's internals for a single
// string capitalization).
function roleLabelForBanner(role) {
...
function initGame(playerStarter, enemyStarter) {
  ...
  enemyUnits = Object.entries(enemyStarter.field).map(([role, id]) =>
    new Unit(UNIT_DEFS[id], ROLE_SLOT[role], "enemy")
  );
...
function renderLockedAbilityButton(ability) {
...
function renderAbilityRow() {
  ...
  if (multiStepMode) {
    ...
    renderLockedAbilityButton(lockedAbility);
    return;
  }
  ...
  selectedUnit.abilities.forEach(ability => {
    ...
    if (ability.passive) {
      ...
      btn.innerHTML = `<div>${ability.name} <span class="ab-speed">Passiva</span></div><div class="ab-desc">${ability.desc}</div>`;
      abilityRow.appendChild(btn);
      return;
    }
    ...
    btn.innerHTML = `<div>${ability.name} <span class="ab-speed">⚡${ability.speed}</span>${cdLabel}</div><div class="ab-desc">${ability.desc}</div>`;
...
function renderTargetPreviewCards(effectiveAbility, pool, targetType) {
  ...
  pool.forEach(u => {
    ...
    card.innerHTML = `
      ...
      <div class="tp-info">
        <div class="tp-name">${u.displayName}</div>
...
function queuePlayerAction(target) {
  ...
  if (multiStepMode) {
    ...
    if (multiStepMode.stepIndex >= multiStepMode.steps.length) {
      ...
      updateStatus(nextStep.kind === 'ability-choice'
        ? `Agora escolha outra habilidade de ${multiStepMode.actor.displayName} para ecoar.`
        : `Agora escolha ${(nextStep.targetType === 'ally') ? 'um aliado' : 'um inimigo'} para ${currentStepAbility().name}.`);
  ...
  if (flow.length > 0) {
    ...
    updateStatus(firstStep.kind === 'ability-choice'
      ? `${ability.name} vai acertar. Agora escolha outra habilidade de ${actor.displayName} para ecoar.`
      : `Agora escolha ${(firstStep.targetType === 'ally') ? 'um aliado' : 'um inimigo'} para ${ability.name}.`);
    renderPartyRow(); renderAbilityRow(); renderTargetRings();
    return;
  }
...
function finalizeMultiStepPlan(lastTarget, { fizzled = false } = {}) {
  ...
  if (fizzled) {
    ...
    updateStatus(`${actor.displayName} has no other ability to echo right now — ${primaryAbility.name} queued without an echo.`);
    ...
    updateStatus(`${primaryAbility.name} + ${chosenAbility.name} queued. Choose an ability for your next unit.`);
  } else {
    ...
    updateStatus(`${primaryAbility.name} queued. Choose an ability for your next unit.`);
  }
  ...
  selectedUnit = null; selectedAbility = null;
  const remaining = livingPlayerUnits().filter(u => !planEntryFor(u));
  if (remaining.length === 0) updateStatus(`All units ready — tap FIGHT! to resolve the round`);
  renderPartyRow(); renderAbilityRow(); renderTargetRings();
}
...
function beginMirrorFlow(ability) {
  ...
  if (!mirroredUnit || !mirroredAbility) {
    ...
    selectedUnit = null; selectedAbility = null;
    updateStatus(`${ability.name} queued — no valid target to mirror right now.`);
    renderPartyRow(); renderAbilityRow(); renderTargetRings();
    return;
  }
  ...
  selectedUnit = null; selectedAbility = null;
  updateStatus(`${ability.name} vai copiar ${mirroredAbility.name} de ${mirroredUnit.displayName}. Escolha o alvo.`);
  renderPartyRow(); renderAbilityRow(); renderTargetRings();
}
...
function chooseMultiStepAbility(ability) {
  ...
  updateStatus(`Choose ${targetType === 'ally' ? 'an ally' : 'an enemy'} for the echoed ${ability.name}`);
  renderAbilityRow();
  renderTargetRings();
}
...
function showOverlay(won) {
  ...
  if (isCampaign) {
    sub.textContent = won ? 'You won this fight.' : 'Your party has fallen.';
    restartBtn.textContent = won ? 'Continue' : 'Return to Menu';
  } else {
    sub.textContent = won
      ? (isPvp ? 'You won the match!' : 'Your squad held the battlefield.')
      : (isPvp ? 'Your opponent won this match.' : 'Your squad has fallen.');
    restartBtn.textContent = isPvp ? 'Back to Menu' : 'Restart Battle';
  }
