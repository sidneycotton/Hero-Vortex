...
const CombatEngine = {
  ...
  applyOne(er, result) {
    ...
    if (er.verb === 'heal') {
      ...
      // Generic "Vig\u00edlia Protegida" amplifier: if any living unit on
      // the heal TARGET's own team holds a healGrowthField status AND
      // currently has shield > 0, this heal raises the target's max HP by
      // the heal amount FIRST, then heals for that same amount against
      // the new ceiling — 80/80 + 5 -> 85/85; 75/80 + 5 -> 75/85 (max
      // raised) -> 80/85 (healed). Checked here (the single funnel every
      // heal verb — heal, healAllAllies, healMarked, spendCounters->heal,
      // sub-cast heals — passes through) rather than per-verb, and
      // applies to self-heals too since it only cares whose TEAM the
      // target is on, not who cast the heal. See StatusLib.healGrowthField
      // in effects.js; first holder is Andressa (js/units/andressa.js).
      const allUnits = [...(result.playerUnits || []), ...(result.enemyUnits || [])];
      const amplifier = allUnits.find(u => u.alive && u.team === t.team && u.shield > 0
        && u.statuses && u.statuses.some(s => s.kind === 'healGrowthField'));
      if (amplifier) t.maxHP += er.amount;
      const before = t.hp;
      t.hp = Math.min(t.maxHP, t.hp + er.amount);
      cleanseOn(t, 'anyHeal'); // e.g. clears Bleed
      return { ...er, actualHeal: t.hp - before, maxHPGrown: amplifier ? er.amount : 0 };
    }
    if (er.verb === 'shield') {
