...
const Net = (() => {
  ...
  function ensurePeerJsLoaded() {
    return new Promise((resolve, reject) => {
      ...
      script.onerror = () => reject(new Error('Could not load networking library (check internet connection).'));
  ...
  async function hostGame({ onStatus } = {}) {
    ...
    if (onStatus) onStatus('Opening room…');
    return new Promise((resolve, reject) => {
      peer = new Peer('herovortex-' + code, { debug: 1 });
      peer.on('open', () => {
        if (onStatus) onStatus('Room open — waiting for opponent…');
        resolve({ code });
      });
    ...
    return new Promise((resolve, reject) => {
      ...
      peer.on('open', () => {
        ...
        const timeout = setTimeout(() => {
          if (!settled) { settled = true; reject(new Error('No response from that room code. Double-check it and that the host is still waiting.')); }
        }, 12000);
        wireConnection(c,
