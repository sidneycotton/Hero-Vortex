...
const StatusLib = {
  ...
  // Generic "grows what it heals" aura: no built-in behavior of its own —
  // the actual payoff (raise the target's max HP by the heal amount FIRST,
  // then heal for that same amount) is checked generically in
  // combat-engine.js's applyOne, right in the shared `heal` branch every
  // heal verb funnels through (heal, healAllAllies, healMarked, spendCounters
  // ->heal, sub-cast heals via useAbilityOn, etc.) — so any future "my
  // presence makes healing permanent" card can reuse this by just applying
  // the status, no per-heal-verb special-casing. The holder does not need
  // to be the one casting the heal, and the heal does not need to target
  // the holder — applyOne checks "is ANY living teammate of the heal's
  // TARGET holding this status AND currently shielded", so it amplifies
  // every heal landing on that whole team, self-heals included. Not
  // time-limited: it's live for as long as the holder both holds this
  // status AND has shield > 0 at the moment a heal actually resolves (no
  // duration/expiry of its own — Andressa's Vigília Protegida re-grants
  // the shield that gates it, not the status itself). First user:
  // Andressa's Vigília Protegida (js/units/andressa.js).
  healGrowthField() {
    return { id: 'heal_growth_field', name: 'Vig\u00edlia Protegida', kind: 'healGrowthField', data: {} };
  },

  // Generic "bonded partner" marker: purely cosmetic/flag status applied to
  // the CHOSEN ally, so its holder is visibly tagged (status pip under its
  // HP bar) as another unit's partner. No built-in behavior of its own —
  // any payoff (damage-sharing, mirrored heals, etc.) is read/implemented
  // by whichever card grants it, keyed off `data.bondedTo`'s displayName.
  // Reusable by any future "link myself to an ally" support passive, not
  // just Andressa's Vínculo da Escudeira (js/units/andressa.js).
  partnerBond(bondedToName) {
    return { id: 'partner_bond', name: 'Parceiro(a)', kind: 'partnerBond', data: { bondedTo: bondedToName } };
  }
};
...
// onUnitSpawned: (unit) => void | Promise<void>
// Fires once for every Unit the moment it takes the field — the starting
// trio at battle start, a forced replacement after a death, a mid-battle
// summon, or a campaign bench swap. This is for passives that need to DO
// something more than "grant myself a permanent status" the instant they
// spawn (that simpler case is still just `ability.passive` — see Unit's
// constructor) — e.g. prompting the player for a choice, like Andressa's
// Vínculo da Escudeira picking a Partner. Registered here instead of
// hand-checking every one of the several `new Unit(...)` call sites so a
// future passive with the same shape gets this for free. Handlers can be
// async (fire-and-forget from the constructor at initial battle spawn,
// where nothing can block; properly awaited at spawn sites that are
// already async, like resolveForcedReplacements in js/planning-ui.js).
function registerUnitSpawnedHook(fn) {
  Hooks.onUnitSpawned.push(fn);
}

async function runUnitSpawnedHooks(unit) {
  for (const fn of Hooks.onUnitSpawned) {
    await fn(unit);
  }
}
...
registerActionResolvedHook(async (action, ctx) => {
  ...
  addLogLine(I18n.t('log.passiveTriggersOff', { unit: UnitText.displayName(holder), actor: UnitText.displayName(action.actor), actorAbility: UnitText.abilityName(action.actor, action.ability), amount: followup.data.amount }), 'hit');
  refreshAllUnitUI();
});
...
registerActionResolvedHook(async (action, ctx) => {
  ...
  addLogLine(I18n.t('log.slowCopyAttack', { unit: UnitText.displayName(action.actor), granter: UnitText.displayName(granter), ability: UnitText.abilityName(granter, grantingAbility) }), 'info');
...
registerActionResolvedHook((action, ctx) => {
  ...
  for (const f of forced) {
    ...
    if (idx !== -1) {
      ...
      addLogLine(I18n.t('log.forcedImmediate', { unit: UnitText.displayName(forcedUnit) }), 'info');
    }
  }
});
...
registerActionResolvedHook((action, ctx) => {
  ...
  for (const rs of roleSwaps) {
    ...
    addLogLine(I18n.t('log.roleSwap', { actor: UnitText.displayName(rs.actor), ally: UnitText.displayName(rs.ally) }), 'info');
  }
});
...
registerStatusAppliedHook('moveLast', (applied, ctx) => {
  ...
  if (idx !== -1) {
    ...
    addLogLine(I18n.t('log.moveLastNow', { unit: UnitText.displayName(delayedUnit) }), 'info');
  }
});
...
registerStatusAppliedHook('slow', (applied, ctx) => {
  ...
  addLogLine(I18n.t('log.nowSlowed', { unit: UnitText.displayName(slowedUnit) }), 'info');
});
